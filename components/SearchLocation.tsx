    ...
    <div className="flex w-full items-center justify-center">
      <Combobox value={location} onChange={setLocation}>
        <div className="relative w-full">
          ...
          <Combobox.Input
            className="h-10 w-full cursor-pointer rounded-md bg-light-white-200 p-4 pl-2 text-xs text-primary-gray-400 outline-none dark:bg-primary-gray-800 dark:text-white xl:text-sm"
            displayValue={(item: string) => item}
            onChange={(event) => setQuery(event.target.value)}
            placeholder="Location - Select your state"
          />
          <Transition
            as={Fragment}
          ...
          >
            <Combobox.Options
              className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-sm shadow-lg focus:outline-none dark:bg-primary-gray-800 sm:text-base"
              static
            >
